...
public class EditCommandParserTest {
    ...
    private static final Password defaultPassword = new Password();
    ...
    public void parse_missingParts_failure() {
        // no index specified
        assertParseFailure(parser, VALID_NAME_AMY + DEFAULT_DESC_ADMINPASSWORD, MESSAGE_INVALID_FORMAT);

        // no field specified
        assertParseFailure(parser, "1" + DEFAULT_DESC_ADMINPASSWORD, EditCommand.MESSAGE_NOT_EDITED);

        // no index and no field specified
        assertParseFailure(parser, "" + DEFAULT_DESC_ADMINPASSWORD, MESSAGE_INVALID_FORMAT);

        // no Password and no field specified
    ...
    public void parse_invalidPreamble_failure() {
        // negative index
        assertParseFailure(parser, "-5" + NAME_DESC_AMY + DEFAULT_DESC_ADMINPASSWORD, MESSAGE_INVALID_FORMAT);

        // zero index
        assertParseFailure(parser, "0" + NAME_DESC_AMY + DEFAULT_DESC_ADMINPASSWORD, MESSAGE_INVALID_FORMAT);

        // invalid arguments being parsed as preamble
        assertParseFailure(parser, "1 some random string" + DEFAULT_DESC_ADMINPASSWORD, MESSAGE_INVALID_FORMAT);

        // invalid prefix being parsed as preamble
        assertParseFailure(parser, "1 i/ string" + DEFAULT_DESC_ADMINPASSWORD, MESSAGE_INVALID_FORMAT);
    }

    @Test
    public void parse_invalidValue_failure() {
        assertParseFailure(parser, "1" + INVALID_NAME_DESC + DEFAULT_DESC_ADMINPASSWORD,
                Name.MESSAGE_NAME_CONSTRAINTS); // invalid name
        assertParseFailure(parser, "1" + INVALID_PHONE_DESC + DEFAULT_DESC_ADMINPASSWORD,
                Phone.MESSAGE_PHONE_CONSTRAINTS); // invalid phone
        assertParseFailure(parser, "1" + INVALID_EMAIL_DESC + DEFAULT_DESC_ADMINPASSWORD,
                Email.MESSAGE_EMAIL_CONSTRAINTS); // invalid email
        assertParseFailure(parser, "1" + INVALID_ADDRESS_DESC + DEFAULT_DESC_ADMINPASSWORD,
                Address.MESSAGE_ADDRESS_CONSTRAINTS); // invalid ptman
        assertParseFailure(parser, "1" + INVALID_TAG_DESC + DEFAULT_DESC_ADMINPASSWORD,
                Tag.MESSAGE_TAG_CONSTRAINTS); // invalid tag


        // invalid phone followed by valid email
        assertParseFailure(parser, "1" + INVALID_PHONE_DESC + EMAIL_DESC_AMY + DEFAULT_DESC_ADMINPASSWORD,
                Phone.MESSAGE_PHONE_CONSTRAINTS);

        // valid phone followed by invalid phone. The test case for invalid phone followed by valid phone
        // is tested at {@code parse_invalidValueFollowedByValidValue_success()}
        assertParseFailure(parser, "1" + PHONE_DESC_BOB + INVALID_PHONE_DESC + DEFAULT_DESC_ADMINPASSWORD,
                Phone.MESSAGE_PHONE_CONSTRAINTS);

        // while parsing {@code PREFIX_TAG} alone will reset the tags of the {@code Employee} being edited,
        // parsing it together with a valid tag results in error
        assertParseFailure(parser, "1" + TAG_DESC_FRIEND + TAG_DESC_HUSBAND + TAG_EMPTY + DEFAULT_DESC_ADMINPASSWORD,
                Tag.MESSAGE_TAG_CONSTRAINTS);
        assertParseFailure(parser, "1" + TAG_DESC_FRIEND + TAG_EMPTY + TAG_DESC_HUSBAND + DEFAULT_DESC_ADMINPASSWORD,
                Tag.MESSAGE_TAG_CONSTRAINTS);
        assertParseFailure(parser, "1" + TAG_EMPTY + TAG_DESC_FRIEND + TAG_DESC_HUSBAND + DEFAULT_DESC_ADMINPASSWORD,
                Tag.MESSAGE_TAG_CONSTRAINTS);

        // multiple invalid values, but only the first invalid value is captured
        assertParseFailure(parser, "1" + INVALID_NAME_DESC + INVALID_EMAIL_DESC + VALID_ADDRESS_AMY + VALID_PHONE_AMY
                + DEFAULT_DESC_ADMINPASSWORD, Name.MESSAGE_NAME_CONSTRAINTS);
    }
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + TAG_DESC_HUSBAND
                + EMAIL_DESC_AMY + ADDRESS_DESC_AMY + NAME_DESC_AMY + TAG_DESC_FRIEND + DEFAULT_DESC_ADMINPASSWORD;

        EditEmployeeDescriptor descriptor = new EditEmployeeDescriptorBuilder().withName(VALID_NAME_AMY)
                .withPhone(VALID_PHONE_BOB).withEmail(VALID_EMAIL_AMY).withAddress(VALID_ADDRESS_AMY)
                .withTags(VALID_TAG_HUSBAND, VALID_TAG_FRIEND).build();
        EditCommand expectedCommand = new EditCommand(targetIndex, descriptor, defaultPassword);
    ...
    public void parse_someFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + EMAIL_DESC_AMY + DEFAULT_DESC_ADMINPASSWORD;
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + NAME_DESC_AMY + DEFAULT_DESC_ADMINPASSWORD;
        EditEmployeeDescriptor descriptor = new EditEmployeeDescriptorBuilder().withName(VALID_NAME_AMY).build();
        EditCommand expectedCommand = new EditCommand(targetIndex, descriptor, defaultPassword);
        assertParseSuccess(parser, userInput, expectedCommand);

        // phone
        userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + DEFAULT_DESC_ADMINPASSWORD;
        descriptor = new EditEmployeeDescriptorBuilder().withPhone(VALID_PHONE_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor, defaultPassword);
        assertParseSuccess(parser, userInput, expectedCommand);

        // email
        userInput = targetIndex.getOneBased() + EMAIL_DESC_AMY + DEFAULT_DESC_ADMINPASSWORD;
        descriptor = new EditEmployeeDescriptorBuilder().withEmail(VALID_EMAIL_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor, defaultPassword);
        assertParseSuccess(parser, userInput, expectedCommand);

        // ptman
        userInput = targetIndex.getOneBased() + ADDRESS_DESC_AMY + DEFAULT_DESC_ADMINPASSWORD;
        descriptor = new EditEmployeeDescriptorBuilder().withAddress(VALID_ADDRESS_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor, defaultPassword);
        assertParseSuccess(parser, userInput, expectedCommand);

        // tags
        userInput = targetIndex.getOneBased() + TAG_DESC_FRIEND + DEFAULT_DESC_ADMINPASSWORD;
    ...
    public void parse_multipleRepeatedFields_acceptsLast() {
        ...
        String userInput = targetIndex.getOneBased()  + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY
                ...
                + PHONE_DESC_BOB + ADDRESS_DESC_BOB + EMAIL_DESC_BOB + TAG_DESC_HUSBAND + DEFAULT_DESC_ADMINPASSWORD;
    ...
    public void parse_invalidValueFollowedByValidValue_success() {
        ...
        Index targetIndex = INDEX_FIRST_EMPLOYEE;
        String userInput = targetIndex.getOneBased() + INVALID_PHONE_DESC + PHONE_DESC_BOB + DEFAULT_DESC_ADMINPASSWORD;
        ...
        userInput = targetIndex.getOneBased() + EMAIL_DESC_BOB + INVALID_PHONE_DESC + ADDRESS_DESC_BOB
                + PHONE_DESC_BOB + DEFAULT_DESC_ADMINPASSWORD;
    ...
    public void parse_resetTags_success() {
        Index targetIndex = INDEX_THIRD_EMPLOYEE;
        String userInput = targetIndex.getOneBased() + TAG_EMPTY + DEFAULT_DESC_ADMINPASSWORD;
