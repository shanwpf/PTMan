...
Currently, there are two commands that allow employers to change attributes of the `OutletInformation`: `editoutlet` and `announcement` commands which are implemented in similar ways.
...
When an `editoutlet` executes successfully, three events will be raised and handled as shown in _<<fig-EditOutletCommandHighLevelSequenceDiagram>>_:
...
. `OutletDataChangedEvent` saves latest outlet information to local storage file "data/outletinformation.xml".
...
When an `encrypt` executes successfully, two events will be raised and handled with reference to _<<fig-EncryptCommandHighLevelSequenceDiagram>>_:

. `PartTimeManagerChangedEvent` saves encrypted or decrypted information of employees and shifts to local storage file "data/parttimemanager.xml".
. `OutletDataChangedEvent` saves encrypted or decrypted outlet information to local storage file "data/outletinformation.xml".
...
The function below demonstrates how outlet data is encrypted using the encrypt(String text) function when converting to a XML file: +
[source, java]
----
