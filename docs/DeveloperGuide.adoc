...
To adhere to defensive programming practices, instead of simply adding the `Employee` to the `Shift`, `ApplyCommand`:
.. Creates a copy of the specified `Shift`
.. Adds the `Employee` to the copy.
.. Replaces the original `Shift` with the copy.
. `UnapplyCommand`: Removes an `Employee` from the `UniqueEmployeeList` in the `Shift`. +
Similar to `ApplyCommand`, `UnapplyCommand`:
.. Creates a copy of the specified `Shift`.
.. Removes the `Employee` from the copy.
...
As seen in _Figure 13_ below, the shift on Monday has index 5 because there are 4 other shifts in the week(s) before the current week.
...
----
This results in the desired shift indexing as shown in _Figure 14_ below.
