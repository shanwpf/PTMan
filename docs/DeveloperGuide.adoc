...
_Figure 1_ below illustrates the high-level design of the app.
...
For example, the `Logic` component, illustrated by _Figure 2_, defines it's API in the `Logic.java` interface and exposes its functionality using the `LogicManager.java` class.
...
.  `Logic` uses the `PartTimeManagerParser` class to parse the user command.
...
* can save the PartTimeManager data in xml format and read it back.
...
_Figure 11_ is a class diagram that displays the association between `Shift` and other components in the `Model`.
...
To store a list of shifts, we use a `UniqueShiftList` to ensure there are no duplicate shifts.
...
The commands `DeleteShiftCommand`, `ApplyCommand` and `UnapplyCommand` access the specified `Shift` via its index.
Having a large number of shifts that are not visible in PTMan will cause the visible shifts to have a large index. To avoid this, we only index shifts that are visible in the current timetable view. +
This can be achieved by setting the `Predicate<Shift>` for the `FilteredList<Shift>` to filter shifts in the current week as shown below:
----
// In Model
...
* `Time` and `Date` uses Java's well-documented `LocalTime` and `LocalDate` classes for easy integration with the GUI timetable.
...
A timetable GUI is useful for employers, as it provides visual aid for them to add and remove shifts from the application.
It is also helpful for both employers and employees to find out which shifts are still available and requires workers. +
...
then allowed to export the timetable as an image for future reference, without having to boot up the application, hence
bringing users convenience.
...
===== Select employee to view working shifts

`TimetablePanel` also subscribes to `EmployeePanelSelectionChangedEvent` to detect if a certain employee is selected. If so,
the timetable will reflect the employee's shifts in a different color by calling `loadEmployeeTimetable(Employee employee)`. +
More to be updated in v1.3.

===== Export timetable to image
...
`UndoRedoStack` only deals with `UndoableCommands`. Commands that cannot be undone will inherit from `Command` instead. The following diagram (_Figure 12_) shows the inheritance diagram for commands:
{nbsp} +
...
{nbsp} +
The following sequence diagram (_Figure 13_) shows how the undo operation works:
...
OutletInformation()
----
This default constructor initializes the seven fields of `OutletInformation` to respective default values:
...
----
This constructor allows flexibility in storing outlet information, but master password must be set to the default value.
...
outlet name, operating hours, contact numbers and email. Master password can only be changed using changemasterpassword command,
while timetable can only be set with relevant `shift` commands.
[NOTE]
...
After login with correct master password, using the "announcement" command, employers should be able to create an announcement.
...
.  Click on the `Print` option in Chrome's menu. You should see a `Print` window as shown in _Figure 14_ below.
.  Set the destination to `Save as PDF`.
.  Click `Save` to save a copy of the file in PDF format. For the best results, use the settings indicated in _Figure 14_.
