...
[NOTE]
Note how the `Model` simply raises a `PartTimeManagerChangedEvent` when the PTMan data are changed, instead of asking the `Storage` to save the updates to the hard disk.
