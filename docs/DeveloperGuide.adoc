...
_Figure 11_ is a class diagram that displays the association between `OutletInformation` and other components in the `Model`.
...
----
This constructor allows flexibility in storing outlet information where most fields can be initialized with specific values except the master password.
The master password of an outlet must be set to the default value because it is used for employer's first login and it is impossible to change any password without the first login. +
...
After login with correct master password, using the `changemasterpassword` command, employers should be able to change the master password.
...
After login with correct master password, using the "announcement" command, employers should be able to create an announcement visible to all employees.
...
You can refer to _Section 4.7 Data Encryption_ for more information on how data is encrypted.
...
_Figure 12_ is a class diagram that displays the association between `Shift` and other components in the `Model`.
...
`UndoRedoStack` only deals with `UndoableCommands`. Commands that cannot be undone will inherit from `Command` instead. The following diagram (_Figure 13_) shows the inheritance diagram for commands:
{nbsp} +
...
{nbsp} +
The following sequence diagram (_Figure 14_) shows how the undo operation works:
...
This implementation successfully prevents any user of the computer directly interpreting the stored information.
However, the limitation is that the XML file structure is still maintained and the title of each element is not encrypted as shown in _Figure 15_.
...
.  Click on the `Print` option in Chrome's menu. You should see a `Print` window as shown in _Figure 16_ below.
