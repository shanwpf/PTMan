...
Moreover, PTMan allows you to encrypt and decrypt data stored locally to prevent any group of people who do not have access to it from interpreting and understanding essential and private data.
...
If the current admin password is `DEFAULT1` and you wish to change it to `hunter22`, type:
...
.Added salary for employee in PTMan +
...
. In the admin mode, you can edit the name, operating hours, contact number, and email of the outlet to be `AwesomeCoffee`, `10am to 11pm`, `98134086`, and `awesomecoffee@gmail.com` respectively, by typing: +
`editoutlet n/AwesomeCoffee h/1000-2300 c/98134086 e/awesomecoffee@gmail.com`. +
You can refer to _<<fig-Edit1>>_.
+
[[fig-Edit1]]
.Example of `editoutlet` command usage +
image::Ui_editoutlet1.png[width="790"]

. Press kbd:[Enter] and you would see a confirmation message below the command bar, and the changes applied to the outlet information.
...
. To only edit the name and contact number to be `Nice Dim Sum` and `901234567` respectively, you can type: `editoutlet n/Nice Dim Sum c/901234567`.
You will see changes as shown in _<<fig-Edit3>>_.
+
[[fig-Edit3]]
.Successful editing of outlet information in PTMan +
image::Ui_editoutlet3.png[width="600"]
...
. You can leave a note for your employees by making an announcement in PTMan. You can type the following command as illustrated in _<<fig-Announce1>>_.
+
[[fig-Announce1]]
...
image::SuccessfulEncryptCommand.jpg[width="500"]
...
. Open `outletinformation.xml` with Internet Explorer (IE). The third line of the file indicates the encryption status with reference to _<<fig-Encrypt>>_.
...
image::SuccessfulDecryptCommand.jpg[width="500"]
...
After your `viewencryption` command is executed successfully, you would see an encryption status message below the command bar as shown in _<<fig-SuccessfulVECommand>>_.

[[fig-SuccessfulVECommand]]
.Example of `viewencryption` command made in PTMan +
image::SuccessfulVECommand.jpg[width="400"]
// end::outletCommand[]
...
Your `INDEX` will be displayed at the right side panel of PTMan. For example, if you are Asther toh, you can execute the command as shown in _<<fig-changingpw>>_.

[[fig-changingpw]]
.Resetting password in PTMan
image::changingpw.png[width="790"]

After executing `changepw` successfully, a notification will be displayed below the command box as shown in _<<fig-changepw>>_.
...
*Format:* `resetpw INDEX` +
*Shorthand:* `rp` `INDEX` +

If you forgot your password, you can simply reset your password with your `INDEX` displayed at the right panel as shown in _<<fig-resetingPw>>_.
[[fig-resetingPw]]
.Resetting password in PTMan
...
After executing `resetpw`, a temporarily password  will be sent to your email registered with your outlet manager as shown in _<<fig-resetPw>>_.

[[fig-resetPw]]
.Reset password in PTMan
...
[TIP]
To stop displaying the employee's shifts on the timetable, use the `defaultview` command. +
Refer to <<Displaying the default timetable: `defaultview`>> for more information.

[[Dv]]
==== Deselecting an employee: `deselect`

Displays the timetable without any employee selected.
Useful for users to navigate back to the default timetable after using the `select` command, or when they have lost track of
where they have navigated the timetable week to. +
...
* <<Dv, *Display default timetable*>>: `defaultview`
